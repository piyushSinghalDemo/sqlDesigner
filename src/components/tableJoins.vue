  ...
  <v-card>
          ...
          <v-card-text>
             <v-form>
               ...
               <v-flex style="margin-right:20px;">
                  <v-select :items="joinType" clearable v-model="tableObj.relationship.selectedFilter" label="Join Type" single-line ></v-select>
               </v-flex>
               <v-flex style="margin-right:20px;">
                   <v-select :items="tableObj.relationship.selectedTableArray" clearable v-model="tableObj.relationship.toTable" 
               ...
               <v-flex xs4 style="margin-right:20px;">
                 <!-- *********************************** Group Column ********************************************* -->
                   <v-select label="From Column" :items="tableObj.optionColumn" clearable v-model="column.fromColumn" item-text="name" 
                     single-line item-value="name + colAlies" autocomplete></v-select>     
                 <!-- ********************************************************************************************** -->
               </v-flex>
               <v-flex xs2 style="margin-right:20px;">
                  <v-select :items="filterArray" v-model="column.operator" clearable label="Operator" single-line ></v-select>
               </v-flex>
               <v-flex xs4 style="margin-right:20px;">
                  <v-layout>
                    <v-flex>
                      <v-select label="To Column" :items="tableObj.optionColumn" clearable v-model="column.toColumn" item-text="name" 
                        single-line item-value="name + colAlies" autocomplete></v-select> 
                    </v-flex>
                  </v-layout>
               </v-flex>
