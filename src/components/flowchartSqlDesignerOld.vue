...
<div class="panel">
            <div class="panel-body" style="margin-top:10px; ">
              ...
              <div class="showdata">
                  <a href="javascript:;" @click="loadDb()">Document</a><span v-if="dragType=='table'"> / {{this.dbOperatorId}}</span>
                  <br><a href="javascript:;" @click="getIDEData()">Get IDE Data</a>

                </div>
                <!-- <div id="minimap"></div> -->
		            <!-- <div id="parent"></div> -->
                <div id="droppable" class="" style="overflow:auto !important;margin-top:10px;">
...
export default {
  ...
  mounted () {
    ...
    $(".draggable").draggable({
      ...
      $("#droppable").droppable({
          ...
          drop: function (event, ui) {
            ...
            if(_this.dragType == 'table')
              return;
            _this.addDbOperator(leftPosition, topPosition, type)
            _this.dragType = 'db'
          }
          ...
          if(type == 'merge'){
            _this.addMergeOperator(leftPosition, topPosition, type)
          }
          if(type == 'if-else'){
            _this.addIfElseOperator(leftPosition, topPosition, type)
          }
          if(type == 'minus'){
            _this.twoInOneOutOperator(leftPosition, topPosition, type)
          }
          if(type == 'purge'){
            _this.oneInZeroOutOperator(leftPosition, topPosition, type)
          }
          if(type == 'archive'){
            _this.oneInZeroOutOperator(leftPosition, topPosition, type)
          }
    ...
    oneInZeroOutOperator(left, top, className){
        ...
        this.dataStr.dbData = $('#droppable').flowchart('getData');
        console.log(JSON.stringify(this.dataStr.dbData));
    },
    oneInOneOutOperator(left, top, className){
      var operatorId = 'created_'+className+'_operator_' + this.operatorI;
      var operatorData = _def.methods.oneInOneOutOperator(this.operatorI, className, top, left);
      this.operatorI += 1;
      $('#droppable').flowchart('createOperator', operatorId, operatorData);
      this.dataStr.workflow[operatorId] = {};
      this.dataStr.dbData = $('#droppable').flowchart('getData');
    ...
    addDbOperator(left, top, className){
      var operatorId = 'created_db_operator_' + this.operatorI;
          var operatorData = {
            new: true,
            className: className,
            top: top,
            left: left,
            properties: {title: 'Step ' + (this.operatorI + 3),inputs: {input_1: {label: '',}},outputs: {output_1: {label: '',}}}
          };
          this.operatorI += 1;
          $('#droppable').flowchart('createOperator', operatorId, operatorData);
          this.dataStr.workflow[operatorId] = {};
    ...
    addIfElseOperator(left, top, className){
      var operatorId = 'created_if-else_operator_' + this.operatorI;
          var operatorData = {
            new: true,
            className: className,
            top: top,
    ...
    addTableOperator(left, top, className){
      var operatorId = 'created_table_operator_' + this.operatorI;
          var operatorData = {
            ...
            top: top,
            left: left,
            properties: {title: 'Table ' + (this.operatorI + 3),inputs: { input_1: {label: 'Input 1',}},outputs: { output_1: {label: 'Output 1',}}}
          };
          this.operatorI += 1;
          $('#droppable').flowchart('createOperator', operatorId, operatorData);
          var data = $('#droppable').flowchart('getData')
          // this.dataStr.workflow[this.dbOperatorId]['dbData'] = data
          this.dataStr.workflow[this.dbOperatorId] = data
          // console.log(JSON.stringify(this.dataStr.workflow));
    },
    addMergeOperator(left, top, className){
      var operatorId = 'created_merge_operator_' + this.operatorI;
          var operatorData = {
            new: true,
            className: className,
            top: top,
            left: left,
            properties: {title: 'Merge Operator ' + (this.operatorI + 3),inputs: {input_1: { label: '',},input_2: { label: '',}}, outputs: { output_1: {label: '',}}}
          };
    ...
    loadDb(){
      console.log(JSON.stringify(this.dbLinkData));
      this.dragType = 'db'
      var linkData = {}
      for (var i = 0; i < this.dbLinkData.length; ++i)
        linkData[i] = this.dbLinkData[i];
      this.dataStr.dbData.links = linkData
      $('#droppable').flowchart('setData', {});
      $('#droppable').flowchart('setData', this.dataStr.dbData);
      // this.applyOperatorClass()
    ...
    loadData(data){
      ...
      $('#droppable').flowchart({
             ...
             onOperatorDoubleClick: function (operatorId) {
               ...
               if(operator.className == 'table'){
                  _this.operatorId = operatorId
                  _this.spForm = operator.data
                  $("#myModal").modal()
               }
               if(operator.className == 'copy'){
                  _this.operatorId = operatorId
                  $("#copyModal").modal()
               }
               if(operator.className == 'archive'){
                  _this.operatorId = operatorId
                  $("#archiveModal").modal()
               }
               if(operator.className == 'purge'){
                  _this.operatorId = operatorId
                  $("#purgeModal").modal()
               }
               if(operator.className == 'report'){
                  _this.operatorId = operatorId
                  $("#reportModal").modal()
               }
               if(operator.className == 'spstep'){
                  _this.operatorId = operatorId
                  $("#spStepModal").modal()
               }
               if(operator.className == 'if-else'){
                  _this.operatorId = operatorId
                  $("#conditionalModal").modal()
               }
               if(operator.className == 'duplicate'){
                  _this.operatorId = operatorId
                  $("#duplicateModal").modal()
               }
               if(operator.className != 'db')
                  return
                console.log("556 double click " + operatorId);
                $(".sb-slidebar").removeClass("toggleshow");

                console.log(JSON.stringify(_this.dataStr.dbData));
                console.log(JSON.stringify(_this.dataStr.workflow));
                $('#droppable').flowchart('getData', {})
             ...
             onLinkCreate: (linkId, linkData) => {
                ...
                if(_this.dragType == 'db'){
                  setTimeout(function(){
                    if(_this.dbLinkData.length > 0){
                      var isExistFlag = false;
                      $.each(_this.dbLinkData, function(i, el){
                        // console.log(JSON.stringify("this data "+el.fromOperator));
                        // console.log(JSON.stringify("linkdata " + el.fromOperator));
                        // console.log(JSON.stringify(el));
                          if((el.fromOperator == linkData.fromOperator) && (el.toOperator == linkData.toOperator)){
                            // console.log(834 + "equal");
                              isExistFlag = true
                          }

                      });
                      if (!isExistFlag) {
                        _this.dbLinkData.push(linkData)
                      }
                    } else {
                      console.log(832);
                      _this.dbLinkData.push(linkData)
                    }
                    // console.log(JSON.stringify(_this.dbLinkData));
                    // _this.dbLinkData.push(linkData)
                    //_this.dataStr.dbData = $('#droppable').flowchart('getData');
                  ...
                  setTimeout(function(){
                    ...
                    // console.log(JSON.stringify(_this.dataStr.dbData));
                    // console.log(JSON.stringify(_this.dataStr.workflow));
                    // _this.dataStr.workflow[_this.dbOperatorId]['dbData'] = data
                    _this.dataStr.workflow[_this.dbOperatorId] = data
...
#droppable { min-height: 400px;height: 100%;width: 105%;border: black 1px solid;
height: 100%;
width: 100%;
overflow: auto;}
