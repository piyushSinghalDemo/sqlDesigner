...
SQL.IO.prototype.serversave = function(e, keyword) {
	...
	var url = CONFIG.HOST_URL + '/v1/sqldesigner/?action=save&keyword='+encodeURIComponent(name)
...
SQL.IO.prototype.serverload = function(e, keyword) {
	...
	var url = CONFIG.HOST_URL + '/v1/sqldesigner/'+encodeURIComponent(name)
	this.owner.window.showThrobber();
	this.name = name;
...
// Vue js
SQL.IO.prototype.serverloadVue = function(data, e, keyword) {
	console.log("515");
	console.log(data);
	var name = keyword || prompt(_("serverloadprompt"), this._name);
	// if (!name) { return; }
	...
	OZ.RequestVue(data, this.loadresponse, {xml:true});
}
